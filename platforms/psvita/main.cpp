...
int main(int argc, char *argv[]) {
    ...
    do {
        ...
        // Initialize Husarnet (adapt from daemon.cpp)
        vita_debug_log("Creating HusarnetManager");
        manager = new HusarnetManager();
        vita_debug_log("HusarnetManager created");

        vita_debug_log("Calling prepareHusarnet");
        manager->prepareHusarnet();
        vita_debug_log("Husarnet prepared");

        vita_debug_log("Setting hostname");
        // Set hostname for join
        Port::setSelfHostname("psvita");
        vita_debug_log("Hostname set");

        // Perform join
        std::string joinCode = "fc94:b01d:1803:8dd8:b293:5c7d:7639:932a/DEYWQGdJYcUBe98A7ZFGce";
        std::string hostname = "psvita";

        vita_debug_log("Getting API address");
        auto apiAddress = manager->configManager->getApiAddress();
        vita_debug_log("API address obtained");
        if (apiAddress.isInvalid()) {
            vita_debug_log("API address is invalid, cannot join");
        } else {
            vita_debug_log("Posting claim");
            auto response = dashboardapi::postClaim(apiAddress, manager->myIdentity, etl::string_view(joinCode.c_str(), joinCode.size()), etl::string_view(hostname.c_str(), hostname.size()));
            vita_debug_log("Claim posted");
            if (response.isSuccessful()) {
                vita_debug_log("Join successful");
            } else {
                vita_debug_log("Join failed: %s", response.toString().c_str());
            }
        }

        vita_debug_log("Joining network: %s with hostname: %s", joinCode.c_str(), hostname.c_str());

        vita_debug_log("Husarnet initialized and joined network");
