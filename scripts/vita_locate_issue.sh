...
VELF_UNPACKED="${ROOTDIR}/cmake-build-husarnet-psv/husarnet-psvita"
VELF_PKG="${ROOTDIR}/cmake-build-husarnet-psv/husarnet-psvita.self"
...
# extract module mappings like: 0x81aca0ab (husarnet_psvita@1 + 0xa8f0ab
module_bases = {}
        ...
        if assigned:
            ...
            if 'husarnet_psvita' in modname:
                if VELF:
                    ...
                    rep.write('  husarnet_psvita module assigned but no local ELF available to resolve DWARF.\n')
            else:
                ...
                if out:
                    ...
                    for idx, line in enumerate(lines):
                        ...
                        for modkey, base in module_bases.items():
                            if base is None: continue
                            # only consider husarnet_psvita modules for mapping into our VELF
                            if not modkey.startswith('husarnet_psvita'):
                                continue
